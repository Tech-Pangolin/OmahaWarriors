...
function Pledges() {
  ...
  const setPledges = async () => {
    for (let pledge of totalPledges) {
      pledge.paid = false;
      pledge.id = `${pledge.name}${Math.floor(Math.random() * 9999999)}`
    }

    try {

      await setDoc(allPledgesRef, {
        totalPledges
      });
    } catch (e) {
      console.log(e)
  ...
  const fetchPost = async () => {
    ...
    const { totalPledges } = pledgeData.data()
    totalPledges.sort((a,b)=>a.player<b.player?-1:1)
  ...
  async function updatePayment(e, pledger) {
    const pledge = totalPledges.find(pledge => pledge.id === pledger.id)
    pledge.paid = e.target.checked;
    try {
      await setDoc(allPledgesRef, {
        totalPledges
      });
  ...
  async function deletePledge(e, pledger) {
    const pledge = totalPledges.findIndex(pledge => pledge.id === pledger.id)
    if (pledge !== -1) {
      totalPledges.splice(pledge, 1)
      setTotalPledges([...totalPledges])
      try {
        await setDoc(allPledgesRef, {
          totalPledges
  ...
  return <main id="main">
    ...
    {showPledges ? (<section id="about" className="about">
      ...
      {email === 'terra.taylor@gmail.com' && <button onClick={setPledges}>Pledge Payment setup</button>}
              ...
              <tbody>
                {totalPledges?.map((player, i) =>
                  <tr key={i}>
                    ...
                    <td>{player?.venmo}</td>
                    <td>{player?.paid} <input className="form-check-input" type="checkbox" defaultChecked={player?.paid} id="flexCheckChecked" onChange={(e) => updatePayment(e, player)} /></td>
                    <td><button type="button" className="btn btn-danger" onClick={(e) => deletePledge(e, player)}>Delete</button></td>
                  </tr>
                )}
              </tbody>
            </table>
